...
export class LunarUtil {
    ...
    static XUN: string[] = ['甲子', '甲戌', '甲申', '甲午', '甲辰', '甲寅'];
    static XUN_KONG: string[] = ['戌亥', '申酉', '午未', '辰巳', '寅卯', '子丑'];
    static LIU_YAO: string[] = ['先胜', '友引', '先负', '佛灭', '大安', '赤口'];
    static HOU: string[] = ['初候', '二候', '三候'];
    static WU_HOU: string[] = ['蚯蚓结', '麋角解', '水泉动', '雁北乡', '鹊始巢', '雉始雊', '鸡始乳', '征鸟厉疾', '水泽腹坚', '东风解冻', '蛰虫始振', '鱼陟负冰', '獭祭鱼', '候雁北', '草木萌动', '桃始华', '仓庚鸣', '鹰化为鸠', '玄鸟至', '雷乃发声', '始电', '桐始华', '田鼠化为鴽', '虹始见', '萍始生', '鸣鸠拂奇羽', '戴胜降于桑', '蝼蝈鸣', '蚯蚓出', '王瓜生', '苦菜秀', '靡草死', '麦秋至', '螳螂生', '鵙始鸣', '反舌无声', '鹿角解', '蜩始鸣', '半夏生', '温风至', '蟋蟀居壁', '鹰始挚', '腐草为萤', '土润溽暑', '大雨行时', '凉风至', '白露降', '寒蝉鸣', '鹰乃祭鸟', '天地始肃', '禾乃登', '鸿雁来', '玄鸟归', '群鸟养羞', '雷始收声', '蛰虫坯户', '水始涸', '鸿雁来宾', '雀入大水为蛤', '菊有黄花', '豺乃祭兽', '草木黄落', '蛰虫咸俯', '水始冰', '地始冻', '雉入大水为蜃', '虹藏不见', '天气上升地气下降', '闭塞而成冬', '鹖鴠不鸣', '虎始交', '荔挺出'];
    static GAN: string[] = ['', '甲', '乙', '丙', '丁', '戊', '己', '庚', '辛', '壬', '癸'];
    static POSITION_XI: string[] = ['', '艮', '乾', '坤', '离', '巽', '艮', '乾', '坤', '离', '巽'];
    static POSITION_YANG_GUI: string[] = ['', '坤', '坤', '兑', '乾', '艮', '坎', '离', '艮', '震', '巽'];
    static POSITION_YIN_GUI: string[] = ['', '艮', '坎', '乾', '兑', '坤', '坤', '艮', '离', '巽', '震'];
    static POSITION_FU: string[] = ['', '巽', '巽', '震', '震', '坎', '离', '坤', '坤', '乾', '兑'];
    static POSITION_FU_2: string[] = ['', '坎', '坤', '乾', '巽', '艮', '坎', '坤', '乾', '巽', '艮'];
    static POSITION_CAI: string[] = ['', '艮', '艮', '坤', '坤', '坎', '坎', '震', '震', '离', '离'];
    static POSITION_TAI_SUI_YEAR: string[] = ['坎', '艮', '艮', '震', '巽', '巽', '离', '坤', '坤', '兑', '坎', '坎'];
    static POSITION_GAN: string[] = ['震', '震', '离', '离', '中', '中', '兑', '兑', '坎', '坎'];
    static POSITION_ZHI: string[] = ['坎', '中', '震', '震', '中', '离', '离', '中', '兑', '兑', '中', '坎'];
    static POSITION_TAI_DAY: string[] = ['占门碓 外东南', '碓磨厕 外东南', '厨灶炉 外正南', '仓库门 外正南', '房床栖 外正南', '占门床 外正南', '占碓磨 外正南', '厕灶厨 外西南', '仓库炉 外西南', '房床门 外西南', '门碓栖 外西南', '碓磨床 外西南', '厨灶碓 外西南', '仓库厕 外正西', '房床炉 外正西', '占大门 外正西', '碓磨栖 外正西', '厨房床 外正西', '仓库碓 外西北', '房床厕 外西北', '占门炉 外西北', '门碓磨 外西北', '厨灶栖 外西北', '仓库床 外西北', '房床碓 外正北', '占门厕 外正北', '碓磨炉 外正北', '厨灶门 外正北', '仓库栖 外正北', '占房床 房内北', '占门碓 房内北', '碓磨厕 房内北', '厨灶炉 房内北', '门仓库 房内北', '床房栖 房内中', '占门床 房内中', '占碓磨 房内南', '厨磨厕 房内南', '仓库炉 房内南', '房床门 房内西', '门碓栖 房内东', '碓磨床 房内东', '厨灶碓 房内东', '仓库厕 房内东', '房床炉 房内中', '占大门 外东北', '碓磨栖 外东北', '厨灶床 外东北', '仓库碓 外东北', '房床厕 外东北', '占门炉 外东北', '门碓磨 外正东', '厨灶栖 外正东', '仓库床 外正东', '房床碓 外正东', '占门厕 外正东', '碓磨炉 外东南', '厨灶门 外东南', '仓库栖 外东南', '占房床 外东南'];
    static POSITION_TAI_MONTH: string[] = ['占房床', '占户窗', '占门堂', '占厨灶', '占房床', '占床仓', '占碓磨', '占厕户', '占门房', '占房床', '占灶炉', '占房床'];
    static ZHI: string[] = ['', '子', '丑', '寅', '卯', '辰', '巳', '午', '未', '申', '酉', '戌', '亥'];
    static ZHI_XING: string[] = ['', '建', '除', '满', '平', '定', '执', '破', '危', '成', '收', '开', '闭'];
    static JIA_ZI: string[] = ['甲子', '乙丑', '丙寅', '丁卯', '戊辰', '己巳', '庚午', '辛未', '壬申', '癸酉', '甲戌', '乙亥', '丙子', '丁丑', '戊寅', '己卯', '庚辰', '辛巳', '壬午', '癸未', '甲申', '乙酉', '丙戌', '丁亥', '戊子', '己丑', '庚寅', '辛卯', '壬辰', '癸巳', '甲午', '乙未', '丙申', '丁酉', '戊戌', '己亥', '庚子', '辛丑', '壬寅', '癸卯', '甲辰', '乙巳', '丙午', '丁未', '戊申', '己酉', '庚戌', '辛亥', '壬子', '癸丑', '甲寅', '乙卯', '丙辰', '丁巳', '戊午', '己未', '庚申', '辛酉', '壬戌', '癸亥'];
    static TIAN_SHEN: string[] = ['', '青龙', '明堂', '天刑', '朱雀', '金匮', '天德', '白虎', '玉堂', '天牢', '玄武', '司命', '勾陈'];
    ...
    static PENGZU_GAN: string[] = ['', '甲不开仓财物耗散', '乙不栽植千株不长', '丙不修灶必见灾殃', '丁不剃头头必生疮', '戊不受田田主不祥', '己不破券二比并亡', '庚不经络织机虚张', '辛不合酱主人不尝', '壬不泱水更难提防', '癸不词讼理弱敌强'];
    static PENGZU_ZHI: string[] = ['', '子不问卜自惹祸殃', '丑不冠带主不还乡', '寅不祭祀神鬼不尝', '卯不穿井水泉不香', '辰不哭泣必主重丧', '巳不远行财物伏藏', '午不苫盖屋主更张', '未不服药毒气入肠', '申不安床鬼祟入房', '酉不会客醉坐颠狂', '戌不吃犬作怪上床', '亥不嫁娶不利新郎'];
    static NUMBER: string[] = ['〇', '一', '二', '三', '四', '五', '六', '七', '八', '九', '十', '十一', '十二'];
    static MONTH: string[] = ['', '正', '二', '三', '四', '五', '六', '七', '八', '九', '十', '冬', '腊'];
    static SEASON: string[] = ['', '孟春', '仲春', '季春', '孟夏', '仲夏', '季夏', '孟秋', '仲秋', '季秋', '孟冬', '仲冬', '季冬'];
    static SHENGXIAO: string[] = ['', '鼠', '牛', '虎', '兔', '龙', '蛇', '马', '羊', '猴', '鸡', '狗', '猪'];
    static DAY: string[] = ['', '初一', '初二', '初三', '初四', '初五', '初六', '初七', '初八', '初九', '初十', '十一', '十二', '十三', '十四', '十五', '十六', '十七', '十八', '十九', '二十', '廿一', '廿二', '廿三', '廿四', '廿五', '廿六', '廿七', '廿八', '廿九', '三十'];
    static YUE_XIANG: string[] = ['', '朔', '既朔', '蛾眉新', '蛾眉新', '蛾眉', '夕月', '上弦', '上弦', '九夜', '宵', '宵', '宵', '渐盈凸', '小望', '望', '既望', '立待', '居待', '寝待', '更待', '渐亏凸', '下弦', '下弦', '有明', '有明', '蛾眉残', '蛾眉残', '残', '晓', '晦'];
    ...
    static XIU_LUCK: Dictionary<string> = new Dictionary<string>([
        ['角', '吉'], ['亢', '凶'], ['氐', '凶'], ['房', '吉'], ['心', '凶'], ['尾', '吉'], ['箕', '吉'], ['斗', '吉'], ['牛', '凶'], ['女', '凶'], ['虚', '凶'], ['危', '凶'], ['室', '吉'], ['壁', '吉'], ['奎', '凶'], ['娄', '吉'], ['胃', '吉'], ['昴', '凶'], ['毕', '吉'], ['觜', '凶'], ['参', '吉'], ['井', '吉'], ['鬼', '凶'], ['柳', '凶'], ['星', '凶'], ['张', '吉'], ['翼', '凶'], ['轸', '吉']
    ]);
    static XIU_POSITION: Dictionary<string> = new Dictionary<string>([
        ['角', '东方'], ['亢', '东方'], ['氐', '东方'], ['房', '东方'], ['心', '东方'], ['尾', '东方'], ['箕', '东方'],['斗', '北方'], ['牛', '北方'], ['女', '北方'], ['虚', '北方'], ['危', '北方'], ['室', '北方'], ['壁', '北方'],['奎', '西方'], ['娄', '西方'], ['胃', '西方'], ['昴', '西方'], ['毕', '西方'], ['觜', '西方'], ['参', '西方'],['井', '南方'], ['鬼', '南方'], ['柳', '南方'], ['星', '南方'], ['张', '南方'], ['翼', '南方'], ['轸', '南方']
    ]);
    ...
    static ZHENG: Dictionary<string> = new Dictionary<string>([
        ['角', '木'], ['井', '木'], ['奎', '木'], ['斗', '木'], ['亢', '金'], ['鬼', '金'], ['娄', '金'], ['牛', '金'], ['氐', '土'], ['柳', '土'], ['胃', '土'], ['女', '土'], ['房', '日'], ['星', '日'], ['昴', '日'], ['虚', '日'], ['心', '月'], ['张', '月'], ['毕', '月'], ['危', '月'], ['尾', '火'], ['翼', '火'], ['觜', '火'], ['室', '火'], ['箕', '水'], ['轸', '水'], ['参', '水'], ['壁', '水']
    ]);
    static ANIMAL: Dictionary<string> = new Dictionary<string>([
        ['角', '蛟'], ['斗', '獬'], ['奎', '狼'], ['井', '犴'], ['亢', '龙'], ['牛', '牛'], ['娄', '狗'], ['鬼', '羊'], ['女', '蝠'], ['氐', '貉'], ['胃', '彘'], ['柳', '獐'], ['房', '兔'], ['虚', '鼠'], ['昴', '鸡'], ['星', '马'], ['心', '狐'], ['危', '燕'], ['毕', '乌'], ['张', '鹿'], ['尾', '虎'], ['室', '猪'], ['觜', '猴'], ['翼', '蛇'], ['箕', '豹'], ['壁', '獝'], ['参', '猿'], ['轸', '蚓']
    ]);
    static GONG: Dictionary<string> = new Dictionary<string>([
        ['角', '东'], ['井', '南'], ['奎', '西'], ['斗', '北'], ['亢', '东'], ['鬼', '南'], ['娄', '西'], ['牛', '北'], ['氐', '东'], ['柳', '南'], ['胃', '西'], ['女', '北'], ['房', '东'], ['星', '南'], ['昴', '西'], ['虚', '北'], ['心', '东'], ['张', '南'], ['毕', '西'], ['危', '北'], ['尾', '东'], ['翼', '南'], ['觜', '西'], ['室', '北'], ['箕', '东'], ['轸', '南'], ['参', '西'], ['壁', '北']
    ]);
    static SHOU: Dictionary<string> = new Dictionary<string>([
        ['东', '青龙'], ['南', '朱雀'], ['西', '白虎'], ['北', '玄武']
    ]);
    static FESTIVAL: Dictionary<string> = new Dictionary<string>([
        ['1-1', '春节'], ['1-15', '元宵节'], ['2-2', '龙头节'], ['5-5', '端午节'], ['7-7', '七夕节'], ['8-15', '中秋节'], ['9-9', '重阳节'], ['12-8', '腊八节']
    ]);
    ...
    static WU_XING_ZHI: Dictionary<string> = new Dictionary<string>([
        ['寅', '木'], ['卯', '木'], ['巳', '火'], ['午', '火'], ['辰', '土'], ['丑', '土'], ['戌', '土'], ['未', '土'], ['申', '金'], ['酉', '金'], ['亥', '水'], ['子', '水']
    ]);
    ...
    static ZHI_HIDE_GAN: Dictionary<string[]> = new Dictionary<string[]>([
        ['子', ['癸']], ['丑', ['己', '癸', '辛']], ['寅', ['甲', '丙', '戊']], ['卯', ['乙']], ['辰', ['戊', '乙', '癸']], ['巳', ['丙', '庚', '戊']], ['午', ['丁', '己']], ['未', ['己', '丁', '乙']], ['申', ['庚', '壬', '戊']], ['酉', ['辛']], ['戌', ['戊', '辛', '丁']], ['亥', ['壬', '甲']]
    ]);
    ...
    static getDayYi(monthGanZhi: string, dayGanZhi: string): string[] {
        ...
        while (index > -1) {
            ...
            let matched = false;
            const months = left.substr(0, left.indexOf(':'));
            let i, m, j;
    ...
    static getDayJiShen(lunarMonth: number, dayGanZhi: string): string[] {
        ...
        if (index > -1) {
            let left = LunarUtil.DAY_SHEN_SHA.substr(index + 4);
            if (left.indexOf('=') > -1) {
                left = left.substr(0, left.indexOf('=') - 3);
            }
            let js = left.substr(0, left.indexOf(','));
            for (let i = 0, j = js.length; i < j; i += 2) {
                l.push(LunarUtil.SHEN_SHA[parseInt(js.substr(i, 2), 16)]);
            }
        }
        if (l.length < 1) {
            l.push('无');
    ...
    static getDayXiongSha(lunarMonth: number, dayGanZhi: string): string[] {
        ...
        if (index > -1) {
            ...
            const xs = left.substr(left.indexOf(',') + 1);
            for (let i = 0, j = xs.length; i < j; i += 2) {
                l.push(LunarUtil.SHEN_SHA[parseInt(xs.substr(i, 2), 16)]);
            }
        }
        if (l.length < 1) {
            l.push('无');
    ...
    static getTimeYi(dayGanZhi: string, timeGanZhi: string): string[] {
        ...
        if (index > -1) {
            let left = LunarUtil.TIME_YI_JI.substr(index + 5);
            if (left.indexOf('=') > -1) {
                left = left.substr(0, left.indexOf('=') - 4);
            }
            let ys = left.substr(0, left.indexOf(','));
            for (let i = 0, j = ys.length; i < j; i += 2) {
                l.push(LunarUtil.YI_JI[parseInt(ys.substr(i, 2), 16)]);
            }
        }
        if (l.length < 1) {
            l.push('无');
    ...
    static getTimeJi(dayGanZhi: string, timeGanZhi: string): string[] {
        ...
        if (index > -1) {
            ...
            let js = left.substr(left.indexOf(',') + 1);
            for (let i = 0, j = js.length; i < j; i += 2) {
                l.push(LunarUtil.YI_JI[parseInt(js.substr(i, 2), 16)]);
            }
        }
        if (l.length < 1) {
            l.push('无');
    ...
    static getXunIndex(ganZhi: string): number {
        const gan = ganZhi.substr(0, 1);
        const zhi = ganZhi.substr(1);
        let ganIndex = 0;
        let zhiIndex = 0;
        let i, j;
        for (i = 0, j = LunarUtil.GAN.length; i < j; i++) {
            if (LunarUtil.GAN[i] === gan) {
                ganIndex = i;
                break;
            }
        }
